  ...
  return (
    <section id="projects-preview" className="w-full bg-[#0b0c0d] text-white border-t border-[#202025]">
      <div className="max-w-6xl mx-auto px-4 py-16">
        <h2 className="uppercase text-left font-space-grotesk font-extrabold text-2xl md:text-3xl tracking-tight mb-6">Featured Projects</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {projects.map((p) => (
            <article key={p.id} className="rounded-md border border-[#26272e] bg-[#181a20] p-5">
              <h3 className="font-space-grotesk font-bold text-lg mb-2">{p.title}</h3>
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs text-[#9ea0a6] font-geist-mono">{p.id}</span>
              </div>
            </article>
          ))}
        </div>
